...
- Všech pět českých diatezí je dostupných přes `SentencePlan.Diathesis`: opisné pasivum, deagentiv a dispoziční diateze (*pracovalo se*, *studentovi se pracovalo*), rezultativ (*mám napsáno*, mít plus neměnné neutrum příčestí) a recipientní deobjektiv (*Karel dostal zaplaceno*, dostat stejnou cestou povyšuje recipienta na podmět). Co ani jedna z posledních dvou nestaví, je příčestí shodující se s vyjádřeným patientem — heslo *Diateze* v NESČ uvádí pod stejným názvem *Novomanželé dostali přidělenu garsonku*, ale české heslo o jmenných tvarech na Wikipedii říká, že takové tvary v šikmých pádech v dnešní češtině přežívají jen jako izolované ustálené příklady, ne jako pravidlo, které by tahle gramatika mohla generovat. Rámce pro diateze, které fungují, jsou ve slovníku u pár sloves každá; opisné pasivum, které mělo jediné sloveso, pokrývá teď čtrnáct významů — odvozených z jejich aktivních rámců pravidlem, které konatele posílá do instrumentálu a patiens do nominativu.
...
- Apozice modelovaná není, takže kontrola jedinečnosti aktantu, kterou `CzechRoleResolver` teď vynucuje, pro ni nemá výjimku — opakovaný funktor se odmítne i tam, kde by ho apozice legitimizovala.
