...
`lexikon.json` is the same file the lexicon tool reads, so a project says where its dictionary is once and both tools find it. The path is taken relative to that file rather than to the working directory, and the file is looked for by walking up, so it works from any subdirectory. The `database` key is read here, and so is `navrhy` — the proposals queue path, same order of precedence, shared by `gramatika`, `rozbor` and `lexikon navrhy` so all three agree on where it is without a project having to say so more than once. The address and the token belong to the tool that talks to the API and are not read here at all.
...
- All five Czech diatheses are reachable through `SentencePlan.Diathesis`: the periphrastic passive, the deagentive and the dispositional (*pracovalo se*, *studentovi se pracovalo*), the resultative (*mám napsáno*, mít plus an invariant neuter participle) and the recipient deobjective (*Karel dostal zaplaceno*, dostat promoting the recipient to subject the same way). What neither of the last two builds is a participle agreeing with an expressed patient — NESČ's *Diateze* entry gives *Novomanželé dostali přidělenu garsonku* under the same name, but Czech Wikipedia's account of jmenné tvary states that such oblique-case short forms survive in today's language only as isolated fixed examples, not a rule this grammar can generate from. Frames for the diatheses that build are in the dictionary for a couple of verbs each; the periphrastic passive, which was down to a single verb, now covers fourteen senses — derived from their active frames by the regular rule, actor to the instrumental and patient to the nominative.
...
- Apposition is not modelled, so the actant-uniqueness check `CzechRoleResolver` now enforces has no exception carved out for it — a repeated functor is refused even where apposition would license one.
